"""Object-oriented interface for DAQmx tasks."""
...
class Task(PyDaqMxTask):
    ...
    def configure_sample_clock(
        ...
        sample_clock_active_edge = daqmx.parameters[
            self.sample_clock_active_edge
        ]
    ...
    def read(self):
        ...
        if self.task_type == "analog input":
            (
                self.newdata,
                self.samples_per_channel_received,
            ) = daqmx.ReadAnalogF64(
                self.handle,
                self.samples_per_channel,
                self.timeout,
        ...
        if self.append_data:
            ...
            if self.time_array:
                newtime = (
                    np.arange(self.samples_per_channel_received + 1)
                    / self.sample_rate
                )
    ...
    def setup_autologging(self, filename, time_array=True, newfile=True):
        ...
        if self.autolog_filetype in ["h5", "hdf5"]:
            ...
            self.data.to_csv(
                self.autolog_file_object, header=True, index=False
            )
    ...
    def do_autolog(self):
        ...
        if self.autolog_filetype == "csv":
            data.to_csv(
                self.autolog_file_object, mode="a", index=False, header=False
            )
    ...
    def setup_append_data(
        self, time_array=True, autotrim=False, autotrim_limit=None
    ):
